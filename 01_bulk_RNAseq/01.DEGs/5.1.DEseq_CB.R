library(DESeq2)
mycounts<-read.csv("CB02_counts_for_DEseq2.txt",sep='\t')
...
summary(res)
write.csv(res,file="All_results_CB02.csv")
diff_gene_deseq2<-subset(res, padj <0.05 & abs(log2FoldChange)>1)
write.csv(diff_gene_deseq2, file="DEG_CB02_tumbler_vs_others.csv")
...
#  coord_fixed()
ggsave(p, filename="PCA_CB.pdf", width=width, height=height)
...
CairoPDF(file="heatmap_sampleToSample_CB.pdf")
