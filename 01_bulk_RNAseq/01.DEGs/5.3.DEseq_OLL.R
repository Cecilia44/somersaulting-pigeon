library(DESeq2)
mycounts<-read.csv("OLL01_counts_for_DEseq2.txt",sep='\t')
...
summary(res)
write.csv(res,file="All_results_OLL01.csv")
diff_gene_deseq2<-subset(res, padj <0.05 & abs(log2FoldChange)>1)
write.csv(diff_gene_deseq2, file="DEG_OLL01_tumbler_vs_others.csv")

library(ggplot2)
vsd=vst(dds)
#rld <- rlog(dds)
pcaData <- plotPCA(vsd,intgroup=c("condition","breed"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
width <- 8
height <- 8
p=ggplot(pcaData, mapping=aes(PC1, PC2, color=breed, shape=breed, label=name)) +
...
#  coord_fixed()
ggsave(p, filename="PCA_OLL.pdf", width=width, height=height)
...
CairoPDF(file="heatmap_sampleToSample_OLL.pdf")
