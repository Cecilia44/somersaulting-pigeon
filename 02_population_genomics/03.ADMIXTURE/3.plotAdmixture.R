...
##============================================================================
## 
header <- "pigeon239_filtered_biAllele_missing1_chrAuto_maf0.01.recode.prune0.1"
max.k <- 20
...
order.fn <- paste(header, "order", sep = ".")
...
my.colours <- c(brewer.pal(8, "Dark2"), "mediumblue", "darkred", "coral4",
                ...
                "grey60", "black", "coral2", "beige")
max.k <- length(plot.data)
n <- dim(plot.data[[1]])[1]
#pdf(file=paste(header, "admix.plot.pdf", sep = "."), width = 16, height = 12)
png(file=paste(header, "admix.plot.png", sep = "."), res=300, width = 8000, height = 4000)
